...
# band_drop_all lives at the repo root (parent of this augmix/ dir).
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from band_dropout_transform import BandDropAll  # noqa: E402
...
parser.add_argument(
    'corrupted_data', metavar='DIR_C', nargs='?', default=None,
    help='path to ImageNet-C dataset (optional; if omitted, corruption '
    'eval / mCE is skipped — useful for de-risk runs on datasets without a '
    'pre-generated -C set)')
parser.add_argument(
    '--aug-method',
    default='augmix',
    choices=['augmix', 'baseline', 'band_drop_all', 'band_drop_all+augmix'],
    help="which training augmentation to use. 'augmix' = original reference "
    "AugMix; 'baseline' = no aug (plain CE, JSD forced off); 'band_drop_all' = "
    "our Laplacian band-dropout on the clean image; 'band_drop_all+augmix' = "
    "reference AugMix then band-dropout on top (normalized-tensor domain).")
parser.add_argument(
    '--band-drop-p', default=0.5, type=float,
    help='per-band dropout probability for band_drop_all methods')
parser.add_argument(
    '--band-mask', default='all', choices=['all', 'hf_only', 'lf_only'],
    help="reviewer-defense control (see docs/REVIEWER_DEFENSE.md). "
    "'all' = default; 'hf_only' / 'lf_only' = drop only top / bottom half.")
parser.add_argument(
    '--optimizer', default='sgd', choices=['sgd', 'adamw'],
    help='sgd (original; CNNs) or adamw (transformers / ConvNeXt)')
parser.add_argument(
    '--schedule', default='step', choices=['step', 'cosine'],
    help='step (original; LR/10 every epochs/3) or cosine (with linear warmup)')
parser.add_argument(
    '--warmup-epochs', default=5, type=int,
    help='linear warmup epochs (only used when --schedule cosine)')
parser.add_argument(
    '--model',
    '-m',
    default='resnet50',
...
def adjust_learning_rate(optimizer, epoch):
  """LR schedule. 'step' = original /10 every epochs/3; 'cosine' = linear
  warmup over --warmup-epochs then cosine decay to 0 over remaining epochs."""
  b = args.batch_size / 256.
  if args.schedule == 'cosine':
    if epoch < args.warmup_epochs:
      m = (epoch + 1) / max(1, args.warmup_epochs)
    else:
      progress = (epoch - args.warmup_epochs) / max(1, args.epochs - args.warmup_epochs)
      m = 0.5 * (1.0 + np.cos(np.pi * progress))
  else:
    k = args.epochs // 3
    if epoch < k:
      m = 1
...
class AugMixDataset(torch.utils.data.Dataset):
  """Dataset wrapper to perform AugMix and/or band_drop_all augmentation.

  aug_method:
    'augmix'               -> reference AugMix (original behavior)
    'baseline'             -> no augmentation (single clean view; JSD off)
    'band_drop_all'        -> Laplacian band-dropout on the clean image
    'band_drop_all+augmix' -> reference AugMix, then band-dropout on top
  """

  def __init__(self, dataset, preprocess, no_jsd=False,
               aug_method='augmix', band_drop=None):
  ...
  def _aug_view(self, x):
    """Produce one augmented, normalized view of PIL image x."""
    if self.aug_method == 'augmix':
      return aug(x, self.preprocess)
    if self.aug_method == 'band_drop_all':
      return self.band_drop(self.preprocess(x))
    if self.aug_method == 'band_drop_all+augmix':
      return self.band_drop(aug(x, self.preprocess))
    # 'baseline' has no augmented view.
    return self.preprocess(x)

  def __getitem__(self, i):
    x, y = self.dataset[i]
    if self.aug_method == 'baseline':
      return self.preprocess(x), y
    if self.no_jsd:
      return self._aug_view(x), y
    im_tuple = (self.preprocess(x), self._aug_view(x), self._aug_view(x))
    return im_tuple, y
...
def main():
  ...
  # 'baseline' is plain CE training: no augmented views, JSD off.
  if args.aug_method == 'baseline':
    args.no_jsd = True
  band_drop = BandDropAll(p=args.band_drop_p, drop_residual=True,
                          band_mask=args.band_mask)
  print("Augmentation method: {} | JSD: {}".format(
      args.aug_method, not args.no_jsd))

  traindir = os.path.join(args.clean_data, 'train')
  valdir = os.path.join(args.clean_data, 'val')
  train_dataset = datasets.ImageFolder(traindir, train_transform)
  train_dataset = AugMixDataset(train_dataset, preprocess, no_jsd=args.no_jsd,
                                aug_method=args.aug_method, band_drop=band_drop)
  ...
  if args.evaluate:
    ...
    if args.corrupted_data is None:
      print('No corrupted_data path given; skipping corruption / mCE eval.')
      return
