...
# band_drop_all lives at the repo root (parent of this pixmix/ dir).
...
parser.add_argument(
    '--aug-method', default='pixmix',
    choices=['pixmix', 'baseline', 'band_drop_all', 'band_drop_all+pixmix'],
    help="training augmentation. 'pixmix' = original; 'baseline' = no aug; "
    "'band_drop_all' = Laplacian band-dropout on clean; "
    "'band_drop_all+pixmix' = PixMix output then band-dropout (normalized).")
parser.add_argument(
...
class PixMixDataset(torch.utils.data.Dataset):
  """Dataset wrapper for PixMix and/or band_drop_all augmentation.

  aug_method:
    'pixmix'               -> original PixMix
    'baseline'             -> no augmentation
    'band_drop_all'        -> Laplacian band-dropout on the clean image
    'band_drop_all+pixmix' -> PixMix output, then band-dropout on top
  """

  def __init__(self, dataset, mixing_set, preprocess,
               aug_method='pixmix', band_drop=None):
    self.dataset = dataset
    self.mixing_set = mixing_set
    self.preprocess = preprocess
    self.aug_method = aug_method
    self.band_drop = band_drop
    self.tensorize = preprocess['tensorize']
    self.normalize = preprocess['normalize']

  def _plain(self, x):
    return self.normalize(self.tensorize(x))

  def __getitem__(self, i):
    x, y = self.dataset[i]
    if self.aug_method == 'baseline':
      return self._plain(x), y
    if self.aug_method == 'band_drop_all':
      return self.band_drop(self._plain(x)), y
    rnd_idx = np.random.choice(len(self.mixing_set))
    mixing_pic, _ = self.mixing_set[rnd_idx]
    out = pixmix(x, mixing_pic, self.preprocess)
    if self.aug_method == 'band_drop_all+pixmix':
      out = self.band_drop(out)
    return out, y
...
def train(net, train_loader, optimizer, scheduler):
  ...
  for i, (images, targets) in enumerate(train_loader):
    ...
    loss.backward()
    # Grad clip for band_drop_all stability; no-op for PixMix/baseline at LR=0.1.
...
def main():
  ...
  print(f"Augmentation method: {args.aug_method} | seed: {args.seed}")
  ...
  train_data = PixMixDataset(train_data, mixing_set,
                             {'normalize': normalize, 'tensorize': to_tensor},
