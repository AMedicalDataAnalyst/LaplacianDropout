...
parser.add_argument(
    ...
    help="which training augmentation to apply per sample. 'pixmix' = original; "
    "'baseline' = no aug; 'band_drop_all' = Laplacian band-dropout on the clean "
    "image; 'band_drop_all+pixmix' = PixMix output then band-dropout on top "
    "(normalized-tensor domain).")
parser.add_argument(
...
class PixMixDataset(torch.utils.data.Dataset):
  ...
  aug_method:
    ...
    'baseline'             -> no augmentation (just preprocess)
...
def main_worker(gpu, args):
    ...
    print("Augmentation method: {}".format(args.aug_method))
    train_dataset = PixMixDataset(train_data, mixing_set,
